CREATE INDEX idx_permission_user_groups_group_expiry ON permission_user_groups(group_name, expires_at_ms);

CREATE OR REPLACE VIEW paradigm_v_players AS
SELECT uuid AS player_uuid, name AS player_name, first_seen_ms, last_seen_ms, playtime_ms
FROM players;

CREATE OR REPLACE VIEW paradigm_v_player_groups AS
SELECT pug.uuid AS player_uuid,
  ...
  CASE WHEN pug.expires_at_ms IS NULL
            OR pug.expires_at_ms > CAST(UNIX_TIMESTAMP(CURRENT_TIMESTAMP(3)) * 1000 AS UNSIGNED)
...
CREATE OR REPLACE VIEW paradigm_v_active_player_groups AS
...
WHERE expires_at_ms IS NULL
   OR expires_at_ms > CAST(UNIX_TIMESTAMP(CURRENT_TIMESTAMP(3)) * 1000 AS UNSIGNED);

CREATE OR REPLACE VIEW paradigm_v_group_members AS
SELECT player_uuid, player_name, group_name, group_description, group_prefix, group_suffix, group_weight,
  assignment_id, assigned_by, assigned_at_ms, expires_at_ms, is_temporary, status, contexts_json, context_hash
FROM paradigm_v_active_player_groups;

CREATE OR REPLACE VIEW paradigm_v_permission_tracks AS
...
CREATE OR REPLACE VIEW paradigm_v_player_tracks AS
...
CREATE OR REPLACE VIEW paradigm_v_player_punishment_summary AS
SELECT p.uuid AS player_uuid, p.name AS player_name,
  COALESCE(SUM(CASE WHEN l.punishment_type = 'BAN' AND l.revoked_at_ms IS NULL
                         AND l.starts_at_ms <= CAST(UNIX_TIMESTAMP(CURRENT_TIMESTAMP(3)) * 1000 AS UNSIGNED)
                         AND (l.expires_at_ms IS NULL OR l.expires_at_ms > CAST(UNIX_TIMESTAMP(CURRENT_TIMESTAMP(3)) * 1000 AS UNSIGNED))
...
CREATE OR REPLACE VIEW paradigm_v_player_ticket_summary AS
...
CREATE OR REPLACE VIEW paradigm_v_servers AS
SELECT server_id, network_id, server_name, created_at_ms, last_seen_ms
FROM server_instances;

CREATE OR REPLACE VIEW paradigm_v_player_home_summary AS
...
CREATE OR REPLACE VIEW paradigm_v_warps_public AS
